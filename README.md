...
По умолчанию CI гоняет `5000` документов из `FiQA` в `auto` режиме, где эмбеддинги строятся самими движками. Полный корпус `57,638` документов оставлен для ручного `workflow_dispatch`, если нужен долгий прогон.
