...
MANTICORE_AUTO_MODEL = "onnx-models/all-MiniLM-L12-v2-onnx"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("auto",),
        default="auto",
        help="Only auto mode is supported; embeddings are generated by the engines on write",
    )
    parser.add_argument(
        "--engines",
    ...
    parser.add_argument(
        "--embed-model",
        default="",
        help="Ignored; retained only for workflow compatibility",
    )
    parser.add_argument(
        "--embed-batch-size",
        type=int,
...
class ManticoreClient:
    ...
    def _bulk_insert(self, table_name: str, batch: list[dict[str, Any]], mode: str) -> Any:
        ...
        bulk_timeout = max(schema_timeout_for_mode(mode, self.timeout), 120.0)
        response = self.session.post(
...
def main() -> int:
    ...
    if args.embed_model or args.embed_batch_size != 128:
        log("[run] embed-model and embed-batch-size are ignored in auto-only benchmark")
